...
vowel_tone = [['á', 'à', 'â', 'ā', 'a'], ['é', 'è', 'ê', 'ē', 'e'],
              ['í', 'ì', 'î', 'ī', 'i'], ['ú', 'ù', 'û', 'ū', 'u'],
              ['ó', 'ò', 'ô', 'ō', 'o']]
...
def gfg():
    if request.method == "POST":
        ...
        mandarin = ''
        if en != None:
            mandarin = GoogleTranslator(
                source='auto', target='zh-TW').translate(en)
        else:
            mandarin = cn
            en = ''
        ...
        for file in files:
            if file[-3:] == 'mp3':
                if int(file[0:2]) <= hour - 1:  # Remove files from 1 hour ago
        ...
        for word in cn_trans:
            ...
            if code == '':
                if word in cleaned_2[:, 0]:
                    ...
                    selector = np.array([word == s for s in cleaned_2[:, 0].flat]).reshape(
                        cleaned_2[:, 0].shape)
                    ...
                    if word in match[:, 1]:
                        code = list(cleaned_2[:, 2])[
                            list(cleaned_2[:, 1]).index(word)]
                        addon = (list(cleaned_2[:, 3])[
                            list(cleaned_2[:, 1]).index(word)]).split('/')
                        ...
                        for p in parts:
                            for m in match[:, 1]:
                                if p in m:  # If part of Mandarin characters match Taiwanese, set the mp3 download code to the number in column E
                                    code = list(cleaned_2[:, 2])[
                                        list(cleaned_2[:, 1]).index(m)]
                                    addon = (list(cleaned_2[:, 3]
                                                  )[list(cleaned_2[:, 1]).index(m)]).split('/')
                                    romanized += addon[0]
            ...
            if code == '':
                if word in cleaned_3_4[:, 0]:
                    selector = np.array([word in s for s in cleaned_3_4[:, 0].flat]).reshape(
                        cleaned_3_4[:, 0].shape)  # Get location of all rows that have search term matching Mandarin exactly
                    ...
                    code = match[1]  # Set mp3 download code to romanization
                    addon = (match[1]).split('/')
                ...
                for s_list in src_1_search:
                    for search_term in s_list:
                        if word == search_term:
                            ...
                            match.append(
                                src_1_tai[src_1_search.index(s_list)])
                            # Concatenate the romanization
                if word in match:
                    code = src_1_code[src_1_tai.index(
                        match[match.index(word)])]
                    addon = (src_1_code[src_1_tai.index(
                        match[match.index(word)])]).split('/')
                    romanized += addon[0]
                elif len(match) > 0:
                    code = src_1_code[src_1_tai.index(match[0])]
                    addon = (
                        src_1_code[src_1_tai.index(match[0])]).split('/')
                    romanized += addon[0]
            ...
            open(f'static/{hour}_{key}_{count}.mp3',
        ...
        return render_template("output.html", en=en, mandarin=mandarin, romanization=romanized, file_count=file_count, hour=hour, key=key, color=color, play=0)
